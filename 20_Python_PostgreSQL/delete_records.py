...
try:
    with psycopg.connect(user='postgres', password='admin', dbname='test_db') as connection:
        with connection.cursor() as cursor:
            ...
            # Delete a record from table persons
            sentence = 'DELETE FROM persons WHERE person_id=(%s)'
            user_input = input('Provide a person_id to delete: ')
            cursor.execute(sentence, (user_input,))
